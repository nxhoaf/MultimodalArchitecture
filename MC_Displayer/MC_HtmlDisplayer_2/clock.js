var Clock = function(spec) {
	...
	that.run = function() {
		that.updateClock();
		document.getElementById(hourPlaceHolder).innerHTML = that.normalize(hours);
		document.getElementById(minutePlaceHolder).innerHTML = that.normalize(minutes);
		document.getElementById(secondPlaceHolder).innerHTML = that.normalize(seconds);
	}
	that.pause = function() {
	...
	that.reset = function() {
		console.log("[clock.js][reset] - Started ");
		seconds = 0;
		hours = 0;
		minutes = 0;
		clearTimeout(t);
		...
		document.getElementById(minutePlaceHolder).innerHTML = that.normalize(minutes);
		document.getElementById(secondPlaceHolder).innerHTML = that.normalize(seconds);
		console.log("[clock.js][reset] - Ended");
	}
