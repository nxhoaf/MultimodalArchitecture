	...
	observator.onTimerElapsed = function() {
		...
		switch (state.getCurrentState()) {
			...
			case ObservatorState.AUTOMATIC_UPDATE_SCHEDULED:
				console.log("[Observator][onTimerElapsed] - AUTOMATIC_UPDATE_SCHEDULED not implemented yet");
				break;
	...
	observator.processEvent = function(state, data) {
		...
		switch(state.getCurrentState()) {
			case ObservatorState.MC_REGISTER_SENT :
				...
				console.log("[Observator][processEvent] - requestId: " + getRequestId());
				returnedState = registerController.processRegister(data, getRequestId());
				state.setCurrentState(returnedState);
				console.log("[Observator][processEvent] - " + state.getStatus());
				...
				sleep = Math.floor(sleep / 1000);
				// time in GPAC is in second, not mili-second
			...
			case ObservatorState.NEW_CONTEXT_REQUEST_SENT:
				...
				console.log("[Observator][processEvent] - " + state.getStatus());
				//alert("******************" + data);
				returnedState = contextController.processNewCtxResponse(data, getRequestId());
				state.setCurrentState(returnedState);
				console.log("[Observator][processEvent] - " + state.getStatus());
				break;
			...
			case ObservatorState.PREPARE_REQUEST_SENT :
				...
				returnedState = contextController.processPrepareResponse(data, getRequestId());
				state.setCurrentState(returnedState);
				console.log("[Observator][processEvent] - " + state.getStatus());
				break;
			...
			case ObservatorState.START_REQUEST_SENT :
				...
				returnedState = contextController.processStartResponse(data, getRequestId());
				state.setCurrentState(returnedState);
				console.log("[Observator][processEvent] - " + state.getStatus());
				break;
			...
			case ObservatorState.POLL_STATUS_SENT :
				...
				returnedState = commandController.processPollStatusResponse(data, getRequestId());
				state.setCurrentState(returnedState);
				console.log("[Observator][processEvent] - " + state.getStatus());
				break;
			...
			case ObservatorState.POLL_PREPARE_SENT:
				...
				returnedState = commandController.processPollPrepareResponse(data, getRequestId());
				state.setCurrentState(returnedState);
				console.log("[Observator][processEvent] - " + state.getStatus());
				break;
			...
			case ObservatorState.POLL_START_SENT :
				...
				returnedState = commandController.processPollStartResponse(data, getRequestId());
				state.setCurrentState(returnedState);
				console.log("[Observator][processEvent] - " + state.getStatus());

				if(state.getCurrentState() == ObservatorState.RECORD_SCHEDULED) {
					startup();
					state.setCurrentState(ObservatorState.POLL_PAUSE_SCHEDULED);
					console.log("[Observator][onTimerElapsed] - " + state.getStatus());
				}
				break;
			...
			case ObservatorState.POLL_PAUSE_SENT :
				...
				returnedState = commandController.processPollPauseResponse(data, getRequestId());
				state.setCurrentState(returnedState);
				console.log("[Observator][processEvent] - " + state.getStatus());
				if(state.getCurrentState() == ObservatorState.PAUSE_SCHEDULED) {
					console.log("[Observator][processEvent] - PAUSE_SCHEDULED: Quit app");
			...
			default:
				console.log("[Observator][processEvent] - Default, state: " + state.getCurrentState() + " *** DO NOTHING ***");
				break;
		}
