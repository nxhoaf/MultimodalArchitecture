package Architecture
{
	...
	public class Controller
	{
		...
		private var isPullMode : Boolean;
		...
		private function checkUpdate() : void {
			trace("[Controller]{checkUpdate} - Started");
		...
		public function onTimerElapsed(event: TimerEvent) : void {
			...
			switch (state.getCurrentState()) 
			{
				...
				case InteractionState.PULL_MODE:
					trace("[Controller]{onTimerElapsed} - "+
						"Pull mode scheduled...");
					checkUpdate();
					break;
		...
		private function processCheckUpdateResponse(state : InteractionState, 
													event : Event) : void {
			trace("[Controller][processCheckUpdateResponse] - Started ");
			// -------------------- 1. Variables declaration  ------------------
			var requestId : String;
			var serviceResponse : URLVariables;
			var type : String;
			...
			switch (type) {
				case MessageType.START:
					trace("[Controller]{processCheckUpdateResponse} - PULL START");
					processStartRequest(requestId);
					break;
				
				case MessageType.PAUSE:
					trace("[Controller]{processCheckUpdateResponse} - PULL PAUSE");
					processPauseRequest(requestId);
					break;
				
				case MessageType.RESUME:
					trace("[Controller]{processCheckUpdateResponse} - PULL RESUME");
					processResumeRequest(requestId);
					break;
				
				case MessageType.CANCEL:
					trace("[Controller]{processCheckUpdateResponse} - PULL CANCEL");
					processCancelRequest(requestId);
					break;
				
				case MessageType.CLEAR_CONTEXT:
					trace("[Controller]{processCheckUpdateResponse} - PULL CLEAR_CONTEXT");
					processClearCtxRequest(requestId);
					break;
				
				default: 
					trace("[Controller]{processCheckUpdateResponse} - Type: " + type);
					break;
			}
			state.setCurrentState(InteractionState.PULL_MODE);
			trace("[Controller]{processCheckUpdateResponse} - " + 
				state.getStatus());
			trace("[Controller][processCheckUpdateResponse] - Ended ");
		}
		...
		private function processEvent(state : InteractionState, 
			...
			switch(state.getCurrentState()) 
			{
				...
				case InteractionState.CHECK_UPDATE_SENT:
		...
		public function switchToCometMode() : void {
			trace("[Controller][switchToCometMode] - Started ");
			var currentState : String = state.getCurrentState();
			isPullMode = false;
			if (currentState == InteractionState.PULL_MODE) {
				trace("[Controller][switchToCometMode] - The system is " + 
					"already in COMET_MODE ");
			} else {
				state.setCurrentState(InteractionState.UI_UPDATE_SCHEDULED);
				trace("[Controller][switchToCometMode] - System changed " + 
					"to  COMET_MODE");
			}
			
			trace("[Controller][switchToCometMode] - Ended ");
		}
		
		public function switchToPullMode() : void {
			trace("[Controller][switchToPullMode] - Started ");
			var currentState : String = state.getCurrentState();
			isPullMode = true;
			if (currentState == InteractionState.PULL_MODE) {
				trace("[Controller][switchToPullMode] - The system is " + 
					"already in PULL_MODE");
			} else {
				state.setCurrentState(InteractionState.PULL_MODE);
				trace("[Controller][switchToPullMode] - System changed " + 
					"to  PULL_MODE");
			}
			trace("[Controller][switchToPullMode] - Ended ");
		}
